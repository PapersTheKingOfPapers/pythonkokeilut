function imgCheck(dict, name){
    let contains = false;
    try {
        contains = name in dict;
    } catch (error){
        contains = false;
    }
    return contains ? dict.medium : "https://via.placeholder.com/210x295?text=Not%20Found";
}
...
tvsearchform.addEventListener('submit', async function(evt) {
    ...
    try {                                               // error handling: try/catch/finally
        ...
        for(let i = 0; i<jsonData.length;i++){
            ...
            img.src = imgCheck(jsonData[i].show.image, "medium");
